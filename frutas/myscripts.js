...
function calcular() {
   ...
   lista.setAttribute('disabled', '') // atributo para deixar em start o modo de pesquisa quando for colocar mais de um item na lista.
    ...
    } else { //se feito todas as atribuições dentro desse else vai aparecer as imagens tbm
    ...
    resultado.innerHTML = `O valor com desconto da fruta ${fruta} é  R$${total.toFixed(2)} `
...
window.addEventListener('load', () => {
    ...
    const frutinha = JSON.parse(data)

    option.text = frutinha.frutaNome
